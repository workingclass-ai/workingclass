...
跑 `/decode`（或者把這段話直接貼給裝了 skill 的 agent），你拿到的是：

```
模式：模糊未來獎勵 + scope-creep 不加薪

這段話真正在說什麼：
  - "broader leadership" / "strategic impact" / "trajectory" 都是不可衡量的期待，
    經理可以隨時重新定義
  - 沒有升遷時間表、沒有薪資 delta、沒有現有 scope 的削減承諾
  - "That's the trajectory I see" ≠ "我們正式承諾給你升職"

你這週該做的：
  1. 回信請經理把**升遷標準**寫下來——具體、可衡量
  2. 問哪些現有的工作可以 drop，騰出時間做新的
  3. 問如果達到那些標準，薪資 delta 是多少
  4. 把經理的回覆（或沉默）存檔——無論哪種都是證據
```

skill 不會叫你**辭職**、不會叫你**和老闆對著幹**、不會叫你"just push back"。它把對方話裡的結構呈現給你，剩下的決定你自己做。
